...
class WaitingForTaskState(State):
    ...
    def execute(self, context):
        current_subtask, started = context.task_manager.get_current_subtask()
...
class ExecutingTaskState(State):
    ...
    def enter(self, context):
        ...
        if current_subtask:
            expected_config = context.state.data['SubtaskConfigs'][subtask_id[1]]
            context.state.update("ExpectedConfig", expected_config)
            logger.info(f"Executing subtask: {subtask_id[1]} with config {expected_config}")
            
            # Notify management interface
            context.management_publisher.send_system_status("executing", f"Executing subtask {subtask_id[1]}")
            context.management_publisher.send_state_change("waiting_for_task", "executing_task")
            context.management_publisher.send_task_update(task_id, subtask_id[1], "started", progress)

        # Send initial task metadata to projector if just started
        context.projector_publisher.send_task(task_id, subtask_id, progress)

    def execute(self, context):
        current_subtask, _ = context.task_manager.get_current_subtask()
...
class WaitingConfirmationState(State):
    ...
    def enter(self, context):
        subtask_id = context.task_manager.get_current_subtask_id()
        logger.info(f"Task rules satisfied for subtask {subtask_id[1]}. Waiting for user confirmation...")
        ...
        last_row = context.config["grid"]["rows"]
        last_col = context.config["grid"]["cols"]
    ...
    def execute(self, context):
        ...
        if left_hand:
            hand_row = left_hand.get('row', 0)
            hand_col = left_hand.get('col', 0)
        ...
        if right_hand and not confirmation_detected:
            hand_row = right_hand.get('row', 0) 
            hand_col = right_hand.get('col', 0)
            if hand_row == last_row and hand_col == last_col:
                confirmation_detected = True
        ...
        if confirmation_detected:
            ...
            context.management_publisher.send_user_action("confirmation_received", {
                "subtask_id": context.task_manager.get_current_subtask_id()[1]
            })
...
class TaskCompletedState(State):
    ...
    def enter(self, context):
        subtask_id = context.task_manager.get_current_subtask_id()
        logger.info(f"All rules satisfied for subtask {subtask_id[1]}")
        
        # Notify management interface
        context.management_publisher.send_system_status("completing", f"Completing subtask {subtask_id[1]}")
    ...
    def execute(self, context):
        ...
        context.projector_publisher.highlight_cell_green(
            context.config["grid"]["rows"],
            context.config["grid"]["cols"]
        )
        ...
        context.task_division_publisher.send_current_subtask_completed(
            subtask_id[1], start_time, end_time
        )
        ...
        context.management_publisher.send_task_update(
            task_id, subtask_id[1], "completed", 100.0
        )
        ...
        context.management_publisher.send_system_status("task_completed", f"Subtask {subtask_id[1]} completed successfully")
